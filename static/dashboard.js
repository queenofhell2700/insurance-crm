...
function initProfileButton() {
    const toggleBtn = document.getElementById('show-user-info-btn');
    ...
    async function fetchUserInfo() {
        try {
            ...
            document.getElementById('info-name').innerText = data.full_name || data.username;
            document.getElementById('info-email').innerText = data.email;
            document.getElementById('info-company').innerText = data.company || 'N/A';
            document.getElementById('info-address').innerText = data.address || 'N/A';
        } catch (error) {
            console.error(error);
            infoCard.innerHTML = "<p style='color: red;'>Error loading profile.</p>";
        }
    }
    ...
    toggleBtn.addEventListener('click', function() {
        if (infoCard.style.display === 'none') {
            ...
            fetchUserInfo();
            toggleBtn.innerText = "Hide Profile Info";
        } else {
            infoCard.style.display = 'none';
            toggleBtn.innerText = "👤 My Profile Info";
        }
    });
}

// ============ NEW: Add Customer Modal Logic ============
function initAddCustomerModal() {
    const modal = document.getElementById('addCustomerModal');
    const openBtn = document.getElementById('add-customer-policy-btn');
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/api/policies/add/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCookie('csrftoken')
                ...
                if (response.ok) {
                    alert('Success! Customer & Policy added.');
                    modal.style.display = 'none';
                    location.reload();
                } else {
